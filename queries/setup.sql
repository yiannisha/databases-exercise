...
-- Create function to log in user
...
-- Create function to log out user
CREATE OR REPLACE FUNCTION logout_user(_username TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    user_id INT;
BEGIN
...
CREATE TYPE JOB_TITLE AS ENUM ('CHEF', 'SOUS_CHEF', 'LINE_COOK', 'PASTRY_CHEF', 'EXECUTIVE_CHEF');
...
CREATE TABLE marks (
    ...
    episode_id INT,
    judge_id INT,
    -- ensure that the chef is participating in the episode
    -- checked with check_chef_in_episode trigger
    chef_id INT REFERENCES chefs(id),
    mark INT CHECK (mark >= 1 AND mark <= 5),
    FOREIGN KEY (episode_id, judge_id) REFERENCES judges (episode_id, judge_id)
);
...
-- Create a function to check if a chef is participating in the episode
CREATE OR REPLACE FUNCTION check_chef_in_episode() RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM episodes_cuisines_chefs
...
-- Create a trigger to check if a chef is participating in the episode
CREATE TRIGGER ensure_chef_in_episode
AFTER INSERT ON marks
FOR EACH ROW EXECUTE FUNCTION check_chef_in_episode();

-- Create a function to check if a recipe is assigned to the chef
CREATE OR REPLACE FUNCTION check_recipe_assigned_to_chef(_username TEXT, _recipe_id INT) RETURNS BOOLEAN AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM recipes
